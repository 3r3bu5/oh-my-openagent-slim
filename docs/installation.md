...
Complete installation instructions for @a4hgehad/oh-my-openagent-slim.
...
By default, the installer is non-destructive. If an `oh-my-openagent-slim.json` configuration file already exists, the installer will **not** overwrite it. Instead, it will display a message:

```
ℹ Configuration already exists at ~/.config/opencode/oh-my-openagent-slim.json. Use --reset to overwrite.
```
...
> **💡 Tip: Models are fully customizable.** The installer sets sensible defaults, but you can assign *any* model to *any* agent. Edit `~/.config/opencode/oh-my-openagent-slim.json` (or `.jsonc` for comments support) to override models, adjust reasoning effort, or disable agents entirely.
...
If you're an LLM Agent helping set up @a4hgehad/oh-my-openagent-slim, follow these steps.
...
bunx @a4hgehad/oh-my-openagent-slim@latest install --no-tui --tmux=yes --skills=yes

# Non-interactive without tmux or skills
bunx @a4hgehad/oh-my-openagent-slim@latest install --no-tui --tmux=no --skills=no
...
- Generates agent model mappings in `~/.config/opencode/oh-my-openagent-slim.json` (or `.jsonc`)
...
- They can easily assign **different models to different agents** by editing `~/.config/opencode/oh-my-openagent-slim.json` (or `.jsonc`).
...
- Read the generated `~/.config/opencode/oh-my-openagent-slim.json` (or `.jsonc`) file to understand the current configuration.
...
```bash
bunx @a4hgehad/oh-my-openagent-slim@latest install --help
```

Then manually create the config files at:
- `~/.config/opencode/oh-my-openagent-slim.json` (or `.jsonc`)
...
2. Verify your config file exists and is valid:
   ```bash
   cat ~/.config/opencode/oh-my-openagent-slim.json
   ```
...
1. **Remove the plugin from your OpenCode config**:
   ...
   Edit `~/.config/opencode/opencode.json` and remove `"@a4hgehad/oh-my-openagent-slim"` from the `plugin` array.

2. **Remove configuration files (optional)**:
   ```bash
   rm -f ~/.config/opencode/oh-my-openagent-slim.json
   rm -f ~/.config/opencode/oh-my-openagent-slim.json.bak
   ```
