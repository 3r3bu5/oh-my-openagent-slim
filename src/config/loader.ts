...
const PROMPTS_DIR_NAME = 'oh-my-openagent-slim';
...
function loadConfigFromPath(configPath: string): PluginConfig | null {
  try {
    ...
    if (!result.success) {
      console.warn(`[oh-my-openagent-slim] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      console.warn(
        `[oh-my-openagent-slim] Error reading config from ${configPath}:`,
        error.message,
      );
    }
    return null;
 ...
 *
 * @param basePath - Base path without extension (e.g., /path/to/oh-my-openagent-slim)
 ...
 * 1. User config: $OPENCODE_CONFIG_DIR/oh-my-openagent-slim.jsonc or .json,
 *    or ~/.config/opencode/oh-my-openagent-slim.jsonc or .json (or $XDG_CONFIG_HOME)
 * 2. Project config: <directory>/.opencode/oh-my-openagent-slim.jsonc or .json
 *
...
export function loadPluginConfig(directory: string): PluginConfig {
  const userConfigBasePath = path.join(getConfigDir(), 'oh-my-openagent-slim');
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[oh-my-openagent-slim] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
  ...
  const readFirstPrompt = (
    ...
    for (const dir of promptSearchDirs) {
      ...
      try {
        ...
        console.warn(
          `[oh-my-openagent-slim] ${errorPrefix} ${promptPath}:`,
