...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'oh-my-openagent-slim', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[oh-my-openagent-slim] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[oh-my-openagent-slim] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[oh-my-openagent-slim] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[oh-my-openagent-slim] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
